...
- `_data/my_experience.yml` — `exps:` list rendered as the timeline on `index.html` and `about.html`. **Keep entries in chronological order (oldest first); both pages `reverse` them for display.** `present: True` marks a currently-held role: it adds the "Present" badge on `index.html` and pins the entry above all past roles on both pages. The ordering is computed in the templates (`reverse` → `where_exp` on `present` → `concat`), so never reorder the YAML to change display order — just append new roles at the end and set `present` correctly. Ending a role means setting `present: False` *and* replacing `PRESENT` in its `period` string with the end month.
